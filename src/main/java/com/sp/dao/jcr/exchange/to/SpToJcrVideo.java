 ...
 */
@Service("SpToJcrVideo")
public class SpToJcrVideo implements SpToJcr<FieldValue<Video>> {

    @Override
    public void accept(FieldValue<Video> spObject, Node node) {
        try {
            Node fileVideos = JcrDaoUtils.addNode(node, FixedNames.binaries());
            JcrDaoUtils.addBinaryToNode(fileVideos,  spObject.getValue(), node.getSession().getValueFactory());
        } catch (Exception e) {
