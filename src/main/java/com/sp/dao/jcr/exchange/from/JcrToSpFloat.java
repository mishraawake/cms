 ...
 */
@Service("JcrToSpFloat")
public class JcrToSpFloat implements JcrToSp<Float> {

    @Autowired
    StringSerialization serialization;

    @Override
    public FieldValue<Float> apply(Node node) {
        try {
            Value value = JcrDaoUtils.getProperty(node, FixedNames.value()).getValue();
            return FieldValue.getFieldValue( JcrDaoUtils.getField(node, serialization) , value.getDecimal().floatValue());
        }catch (Exception e){
