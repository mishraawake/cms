 ...
 */
@Service("SpToJcrArrayOfFile")
public class SpToJcrArrayOfFile implements SpToJcr<FieldValue<FileObject[]>> {

    @Override
    public void accept(FieldValue<FileObject[]> spObject, Node node) {
        try {
            Node fileObjects = JcrDaoUtils.addNode(node, FixedNames.binaries());
            JcrDaoUtils.addBinariesToNode(fileObjects, spObject.getValue(), node.getSession().getValueFactory());
        } catch (Exception e) {
