 ...
 */
@Service("SpToJcrImage")
public class SpToJcrImage implements SpToJcr<FieldValue<Image>> {

    @Override
    public void accept(FieldValue<Image> spObject, Node node) {
        try {
            Node fileImages = JcrDaoUtils.addNode(node, FixedNames.binaries());
            JcrDaoUtils.addBinaryToNode(fileImages, spObject.getValue(), node.getSession().getValueFactory());
        } catch (Exception e) {
